"""
TAGE-Loop: TAGE with Dedicated Loop Predictor

Key insight: Loop branches (backward jumps with fixed iteration counts) are 
highly predictable but TAGE struggles with them because:
1. The pattern repeats exactly every N iterations
2. TAGE's history-based approach doesn't capture iteration counts well

The Loop Predictor specifically tracks:
- Loop iteration count
- Total iterations before exit
- Confidence in the loop pattern

Override TAGE only for HIGH-CONFIDENCE loop predictions.

This is based on the loop predictor component from TAGE-SC-L (CBP winner).
...
TAGE_LOOP_64KB = {
    'name': 'TAGE-Loop-64KB',
    # TAGE parameters (same as original 64KB)
    'n_history_tables': 12,
    'min_hist': 4,
    'max_hist': 640,
    ...
    'loop_table_size': 128,  # Number of loop entries
    'loop_tag_bits': 14,     # Tag bits for loop entries  
    'loop_iter_bits': 14,    # Bits for iteration counter (max 16K iterations)
    'loop_conf_bits': 3,     # Confidence counter bits
    'loop_age_bits': 8,      # Age counter for replacement
}
...
class LoopEntry:
    ...
    Tracks loop iteration patterns:
    - current_iter: Current iteration count within the loop
    - past_iter: Number of iterations in previous complete loop execution
    - age: For replacement policy
    - confidence: How confident we are this is a real loop
    - tag: For matching
    - dir: Direction of the loop (taken to stay in loop)
    """
    __slots__ = ['tag', 'current_iter', 'past_iter', 'age', 'confidence', 'dir']
    ...
    def __init__(self):
        ...
        self.age = 0
        self.confidence = 0
        self.dir = True  # True = taken stays in loop


class TAGE_Loop(BasePredictor):
    """
    TAGE with Dedicated Loop Predictor.
    
    The loop predictor identifies branches that follow a loop pattern
    (taken N times, then not-taken once, repeat) and predicts them
    based on iteration counting rather than history correlation.
    
    Key features:
    1. Standard TAGE as base predictor
    2. Loop predictor runs in parallel
    3. Loop predictor overrides TAGE only when:
       - High confidence in loop pattern
       - Current iteration count matches expected
    4. Very conservative - only overrides when very sure
    """
    
    def __init__(self, config: dict = None):
        config = config or TAGE_LOOP_64KB
        name = config.get('name', 'TAGE-Loop')
        super().__init__(name, config)
        
        # === TAGE Core Parameters ===
        ...
        self.num_use_alt_on_na = config.get('num_use_alt_on_na', 128)
        ...
        # === Loop Predictor Parameters ===
        self.loop_table_size = config.get('loop_table_size', 128)
        self.loop_tag_bits = config.get('loop_tag_bits', 14)
        self.loop_iter_bits = config.get('loop_iter_bits', 14)
        self.loop_conf_bits = config.get('loop_conf_bits', 3)
        self.loop_age_bits = config.get('loop_age_bits', 8)
        
        self.loop_iter_max = (1 << self.loop_iter_bits) - 1
        self.loop_conf_max = (1 << self.loop_conf_bits) - 1
        self.loop_age_max = (1 << self.loop_age_bits) - 1
        self.loop_tag_mask = (1 << self.loop_tag_bits) - 1
        
        # TAGE derived parameters
        ...
        self.loop_table = [LoopEntry() for _ in range(self.loop_table_size)]
        
        # Track when loop predictor is useful
        self.loop_use_counter = 0  # + when loop right & TAGE wrong, - otherwise
    ...
    def _get_bimode_pred(self, pc: int) -> bool:
        return self.btable_prediction[self._bindex(pc)]
    ...
    def _loop_index(self, pc: int) -> int:
        """Get loop table index."""
        return (pc >> self.inst_shift_amt) & (self.loop_table_size - 1)
    
    def _loop_tag(self, pc: int) -> int:
        """Get loop table tag."""
        return (pc >> (self.inst_shift_amt + 7)) & self.loop_tag_mask
    
    def _loop_predict(self, pc: int) -> tuple:
        """
        Get loop predictor prediction.
        Returns: (valid, prediction, confidence)
        """
        idx = self._loop_index(pc)
        ...
        if entry.confidence < self.loop_conf_max:
            return (False, False, entry.confidence)
        
        # Need valid past iteration count
        if entry.past_iter == 0:
            return (False, False, entry.confidence)
        
        # Predict based on iteration count
        # If current_iter < past_iter, predict taken (stay in loop)
        # If current_iter >= past_iter, predict not-taken (exit loop)
        if entry.current_iter < entry.past_iter:
            pred = entry.dir  # Stay in loop
        else:
            pred = not entry.dir  # Exit loop
        
        return (True, pred, entry.confidence)
    
    def _loop_update(self, pc: int, taken: bool, tage_pred: bool):
        """Update loop predictor entry."""
        idx = self._loop_index(pc)
        tag = self._loop_tag(pc)
        entry = self.loop_table[idx]
        
        if entry.tag != tag:
            # Tag mismatch - consider allocation
            # Only allocate if this looks like a loop start (taken branch)
            if taken and entry.age < self.loop_age_max // 2:
                # Allocate new entry
                entry.tag = tag
                ...
                entry.confidence = 0
                entry.age = 0
                entry.dir = True  # Assume taken = stay in loop
            else:
                # Age existing entry
                entry.age = min(entry.age + 1, self.loop_age_max)
            return
        
        # Tag match - update entry
        entry.age = 0  # Reset age on access
        
        if taken == entry.dir:
            # Staying in loop
            if entry.current_iter < self.loop_iter_max:
                entry.current_iter += 1
        else:
            # Exiting loop
            if entry.past_iter == 0:
                # First time seeing loop end - record iteration count
                entry.past_iter = entry.current_iter
                if entry.confidence < self.loop_conf_max:
                    entry.confidence += 1
            elif entry.current_iter == entry.past_iter:
                # Iteration count matches - increase confidence
                if entry.confidence < self.loop_conf_max:
                    entry.confidence += 1
            else:
                # Iteration count mismatch - decrease confidence
                if entry.current_iter < entry.past_iter:
                    # Loop exited early - update past_iter
                    entry.past_iter = entry.current_iter
                    entry.confidence = max(0, entry.confidence - 1)
                else:
                    # Loop ran longer than expected
                    entry.past_iter = entry.current_iter
                    entry.confidence = max(0, entry.confidence - 1)
    ...
    def predict(self, pc: int, history: np.ndarray = None) -> PredictionResult:
        ...
        if hit_bank > 0:
            ...
            tage_confidence = abs(self.gtable[hit_bank-1][hit_idx].ctr) / self.counter_max
        else:
            alt_taken = self._get_bimode_pred(pc)
        ...
        # === Loop Predictor ===
        loop_valid, loop_pred, loop_conf = self._loop_predict(pc)
        
        # === Final Decision ===
        # Use loop predictor only if:
        # 1. Loop predictor is valid (tag match, high confidence, valid past_iter)
        # 2. Loop predictor disagrees with TAGE
        # 3. TAGE confidence is not very high
        use_loop = False
        if loop_valid and loop_pred != tage_pred:
            # Only override if TAGE is not very confident
            if tage_confidence < 0.8:
                use_loop = True
        ...
        return PredictionResult(
            ...
            confidence=tage_confidence if not use_loop else float(loop_conf) / self.loop_conf_max,
    ...
    def update(self, pc: int, history: np.ndarray, taken: bool, 
        ...
        use_loop = info['use_loop']
        loop_pred = info['loop_pred']
        
        # Track loop predictor usefulness
        if use_loop:
            if loop_pred == taken and tage_pred != taken:
                self.loop_use_counter = min(127, self.loop_use_counter + 1)
            elif loop_pred != taken:
                self.loop_use_counter = max(-128, self.loop_use_counter - 1)
    ...
    def get_hardware_cost(self) -> dict:
        ...
        # Loop predictor
        bits_per_loop = (self.loop_tag_bits + 
                        self.loop_iter_bits * 2 +  # current + past
                        self.loop_conf_bits +
                        self.loop_age_bits + 1)  # +1 for dir
        bits += self.loop_table_size * bits_per_loop
